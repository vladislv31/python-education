...
from models import GameBoard, Player, AIPlayer
...
class Controller:
    ...
    def start_game(self):
        ...
        player_name = self.view.input_string_field('your name')

        player = Player(player_name, 'X')
        ai_player = AIPlayer('AI', 'O')

        game_board = GameBoard([player, ai_player], logs_file=config.LOGS_FILE)
        ...
        while True:
            ...
            for player in game_board.get_players_queue():
                ...
                if isinstance(player, AIPlayer):
                    move = player.find_move(game_board)
                    game_board.apply_move(move['index'], player.get_team())
                    self.view.player_applied_move(player, move['index'])
                    press_key()
                else:
